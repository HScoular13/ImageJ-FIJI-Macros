macro "Full_Histology_Image_Processor" {
	...
	main_dir_prompt = getBoolean("Select the folder containing the image folder");
	main_dir = getDir("Choose Main Directory Containing the Image Folder");
	im_dir_prompt = getBoolean("Choose folder containing images to be processed");
...
//				Array.print(tissue_top_y);
		...
		saveAs("tiff", main_dir + stack_name);
		close("*");
		run("Collect Garbage");
		run("Collect Garbage");
